...
Header: X-Claude-API-Key: claude_99efff1a004422bdb67acf3f345f8a20e4fe8c29a734a82c132b2500d9fbd4bf
Body: { "query": "SELECT started_at, duration_minutes, total_calls, connected, interested, ai_recap, hot_leads, follow_up_plan FROM dialer_session_logs WHERE rep_id = 'admin' ORDER BY started_at DESC LIMIT 3" }
```
...
Body: {"assignedTo":"Admin","tags":["sba interest"],"industry":"construction","limit":200}
```
...
| Rep says | Body |
|---|---|
| "SBA leads" | `{"assignedTo":"Admin","tags":["sba"],"limit":200}` |
| "construction leads with revenue" | `{"assignedTo":"Admin","industry":"construction","monthlyRevenueMin":"notempty"}` |
| "trucking in California" | `{"assignedTo":"Admin","industry":"trucking","state":"CA"}` |
| "west coast leads" | `{"assignedTo":"Admin","state":"CA,WA,OR,NV,AZ"}` |
| "Florida leads" | `{"assignedTo":"Admin","state":"FL"}` |
| "212 area code" | `{"assignedTo":"Admin","areaCodes":["212"]}` |
| "UCC leads" | `{"assignedTo":"Admin","tags":["ucc"]}` |
| "top tier prospects" | `{"assignedTo":"Admin","tags":["top tier prospects"]}` |
| "leads with approvals" | `{"assignedTo":"Admin","hasApproval":true}` |
| "no answers from last time" | `{"assignedTo":"Admin","lastDisposition":"No Answer"}` |
| "fresh leads never called" | `{"assignedTo":"Admin","neverContacted":true}` |
| "leads in underwriting" | `{"assignedTo":"Admin","sfOppStage":"Underwriting"}` |
| "leads with SF records" | `{"assignedTo":"Admin","hasSfRecord":true}` |
...
POST https://app.todaycapitalgroup.com/api/admin/claude/sql
Header: X-Claude-API-Key: claude_99efff1a004422bdb67acf3f345f8a20e4fe8c29a734a82c132b2500d9fbd4bf
Body: { "query": "SELECT * FROM dialer_contacts WHERE assigned_to = 'Admin' AND phone != '' AND [FILTERS] ORDER BY last_contacted ASC NULLS FIRST LIMIT 200" }
```
...
curl -s -X POST "$DASHBOARD_URL/api/dialer/start" \
  ...
  -d '{"repId":"admin","repName":"Admin","leads":[...leads...],"connectionMode":"webrtc"}'
```
...
Body: {
  ...
  "params": ["SESSION_ID","admin","Admin","START_ISO","END_ISO",MINS,LEADS,CALLS,CONNECTED,INTERESTED,CALLBACKS,NOT_INT,NO_ANS,VM,TALK_SECS,"single","webrtc","source description","[{hot leads JSON}]","[\"follow up items\"]","2-3 sentence recap","[{call details JSON}]"]
}
```
...
2. **Admin has access to all reps' leads** — no assigned_to filter needed
