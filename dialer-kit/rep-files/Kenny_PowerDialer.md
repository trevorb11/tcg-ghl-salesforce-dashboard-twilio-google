...
Header: X-Claude-API-Key: claude_99efff1a004422bdb67acf3f345f8a20e4fe8c29a734a82c132b2500d9fbd4bf
Body: { "query": "SELECT started_at, duration_minutes, total_calls, connected, interested, ai_recap, hot_leads, follow_up_plan FROM dialer_session_logs WHERE rep_id = 'kenny' ORDER BY started_at DESC LIMIT 3" }
```
...
Body: {"assignedTo":"Kenny Nwobi","tags":["sba interest"],"industry":"construction","limit":200}
```
...
| Rep says | Body |
|---|---|
| "SBA leads" | `{"assignedTo":"Kenny Nwobi","tags":["sba"],"limit":200}` |
| "construction leads with revenue" | `{"assignedTo":"Kenny Nwobi","industry":"construction","monthlyRevenueMin":"notempty"}` |
| "trucking in California" | `{"assignedTo":"Kenny Nwobi","industry":"trucking","state":"CA"}` |
| "west coast leads" | `{"assignedTo":"Kenny Nwobi","state":"CA,WA,OR,NV,AZ"}` |
| "Florida leads" | `{"assignedTo":"Kenny Nwobi","state":"FL"}` |
| "212 area code" | `{"assignedTo":"Kenny Nwobi","areaCodes":["212"]}` |
| "UCC leads" | `{"assignedTo":"Kenny Nwobi","tags":["ucc"]}` |
| "top tier prospects" | `{"assignedTo":"Kenny Nwobi","tags":["top tier prospects"]}` |
| "leads with approvals" | `{"assignedTo":"Kenny Nwobi","hasApproval":true}` |
| "no answers from last time" | `{"assignedTo":"Kenny Nwobi","lastDisposition":"No Answer"}` |
| "fresh leads never called" | `{"assignedTo":"Kenny Nwobi","neverContacted":true}` |
| "leads in underwriting" | `{"assignedTo":"Kenny Nwobi","sfOppStage":"Underwriting"}` |
| "leads with SF records" | `{"assignedTo":"Kenny Nwobi","hasSfRecord":true}` |
...
POST https://app.todaycapitalgroup.com/api/admin/claude/sql
Header: X-Claude-API-Key: claude_99efff1a004422bdb67acf3f345f8a20e4fe8c29a734a82c132b2500d9fbd4bf
Body: { "query": "SELECT * FROM dialer_contacts WHERE assigned_to = 'Kenny Nwobi' AND phone != '' AND [FILTERS] ORDER BY last_contacted ASC NULLS FIRST LIMIT 200" }
```
...
curl -s -X POST "$DASHBOARD_URL/api/dialer/start" \
  ...
  -d '{"repId":"kenny","repName":"Kenny Nwobi","leads":[...leads...],"connectionMode":"webrtc"}'
```
...
curl -s -X POST "$DASHBOARD_URL/api/auth/token" \
  -H "X-Dialer-Key: $DIALER_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{"email":"kenny@todaycapitalgroup.com","phone":"","sessionId":"SESSION_ID"}'
```
...
Body: {
  ...
  "params": ["SESSION_ID","kenny","Kenny Nwobi","START_ISO","END_ISO",MINS,LEADS,CALLS,CONNECTED,INTERESTED,CALLBACKS,NOT_INT,NO_ANS,VM,TALK_SECS,"single","webrtc","source description","[{hot leads JSON}]","[\"follow up items\"]","2-3 sentence recap","[{call details JSON}]"]
}
```
...
2. **Only access this rep's leads** — filter by `assigned_to = 'Kenny Nwobi'`
