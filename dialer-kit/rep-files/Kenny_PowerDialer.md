...
curl -s -X POST "$DASHBOARD_URL/api/leads/query" \
  ...
  -d '{"tags":["sba interest"],"industry":"construction","assignedTo":"Kenny Nwobi","monthlyRevenueMin":"notempty","limit":200}'
```
...
Query `dialer_contacts` directly. **Always filter by `assigned_to = 'Kenny Nwobi'`.**

```
POST https://app.todaycapitalgroup.com/api/admin/claude/sql
Header: X-Claude-API-Key: claude_99efff1a004422bdb67acf3f345f8a20e4fe8c29a734a82c132b2500d9fbd4bf
Body: { "query": "SELECT ghl_contact_id, first_name, last_name, phone, email, business_name, opp_stage_selection, tags, monthly_revenue, industry_dropdown, last_note, call_disposition, sf_opportunity_id, sf_opp_stage, sf_contact_id FROM dialer_contacts WHERE assigned_to = 'Kenny Nwobi' AND phone IS NOT NULL AND phone != '' AND (dnd IS NULL OR dnd = '' OR dnd = 'false') AND [FILTERS] ORDER BY last_contacted ASC NULLS FIRST LIMIT 200" }
```
