# TCG Power Dialer — Gregory Dergevorkian
...
Body: {"assignedTo":"Gregory Dergevorkian","tags":["sba interest"],"industry":"construction","limit":200}
```
...
| Filter | Type | Example |
|---|---|---|
| `assignedTo` | string | `"Gregory Dergevorkian"` |
...
POST https://app.todaycapitalgroup.com/api/admin/claude/sql
Header: X-Claude-API-Key: claude_99efff1a004422bdb67acf3f345f8a20e4fe8c29a734a82c132b2500d9fbd4bf
Body: { "query": "SELECT * FROM dialer_contacts WHERE assigned_to = 'Gregory Dergevorkian' AND phone != '' AND [FILTERS] ORDER BY last_contacted ASC NULLS FIRST LIMIT 200" }
```
...
curl -s -X POST "$DASHBOARD_URL/api/dialer/start" \
  ...
  -d '{"repId":"gregory","repName":"Gregory Dergevorkian","leads":[...leads...],"connectionMode":"webrtc"}'
```
...
Body: {
  ...
  "params": ["SESSION_ID","gregory","Gregory Dergevorkian","START_ISO","END_ISO",MINS,LEADS,CALLS,CONNECTED,INTERESTED,CALLBACKS,NOT_INT,NO_ANS,VM,TALK_SECS,"single","webrtc","source description","[{hot leads JSON}]","[\"follow up items\"]","2-3 sentence recap","[{call details JSON}]"]
}
```
...
2. **Only access this rep's leads** — filter by `assigned_to = 'Gregory Dergevorkian'`
