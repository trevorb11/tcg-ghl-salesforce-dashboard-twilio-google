# TCG Power Dialer — Claude Operator File (TEMPLATE)

> **TEMPLATE — Copy this file for each rep and fill in their specific values.**
...
This is **REP_NAME_HERE** (`REP_EMAIL_HERE`). Their rep ID is `REP_ID_HERE`. Here's the instant startup flow:
...
curl -s -X POST "$DASHBOARD_URL/api/auth" \
  -H "X-Dialer-Key: $DIALER_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{"email": "REP_EMAIL_HERE", "phone": "REP_PHONE"}'
```
...
**CRITICAL: Always filter by `assigned_to = 'REP_NAME_HERE'`** — this rep can only dial their own leads. Never return leads assigned to other reps.
...
REP_ID = "REP_ID_HERE"
REP_NAME = "REP_NAME_HERE"
REP_EMAIL = "REP_EMAIL_HERE"
...
cur.execute("""
    ...
    WHERE assigned_to = 'REP_NAME_HERE'
...
9. **Only access this rep's leads** — always filter by `assigned_to = 'REP_NAME_HERE'` in DB queries
