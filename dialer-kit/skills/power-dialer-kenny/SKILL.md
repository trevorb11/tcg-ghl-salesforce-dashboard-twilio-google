...
This is **Kenny Nwobi** (`kenny@todaycapitalgroup.com`). Their rep ID is `kenny`. Here's the instant startup flow:
...
curl -s -X POST "$DASHBOARD_URL/api/auth" \
  ...
  -d '{"email": "kenny@todaycapitalgroup.com", "phone": ""}'
```
...
curl -s -X POST "$DASHBOARD_URL/api/auth" \
  -H "X-Dialer-Key: $DIALER_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{"email": "kenny@todaycapitalgroup.com", "phone": "REP_PHONE"}'
```
...
curl -s -X POST "$DASHBOARD_URL/api/auth/token" \
  -H "X-Dialer-Key: $DIALER_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{"email": "kenny@todaycapitalgroup.com", "phone": "REP_PHONE", "sessionId": "SESSION_ID"}'
```
...
9. **Only access this rep's leads** — always filter by `assigned_to = 'Kenny Nwobi'` in DB queries
                                                                 