---
name: power-dialer-julius
description: >
  TCG Power Dialer operator for Julius Speck. Use this skill whenever the user mentions dialing, calling, leads, outbound calls, power dialer, session, pipeline, CRM, GoHighLevel, GHL, contacts, dispositions, voicemail, callbacks, MCA, merchant cash advance, funding leads, or anything related to making sales calls. Also trigger when the user says "start dialing", "load my leads", "let's call", "dial my list", "absent leads", "MIA leads", "new opps", "follow ups", "hold list", or asks about call stats, session summaries, or daily briefings. This skill contains all credentials, API endpoints, and dialer instructions for Julius Speck.
---
...
This is **Julius Speck** (`julius@todaycapitalgroup.com`). Their rep ID is `julius`. Here's the instant startup flow:
...
curl -s -X POST "$DASHBOARD_URL/api/auth" \
  ...
  -d '{"email": "julius@todaycapitalgroup.com", "phone": ""}'
```
...
curl -s -X POST "$DASHBOARD_URL/api/auth" \
  -H "X-Dialer-Key: $DIALER_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{"email": "julius@todaycapitalgroup.com", "phone": "REP_PHONE"}'
```
...
curl -s -X POST "$DASHBOARD_URL/api/auth/token" \
  -H "X-Dialer-Key: $DIALER_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{"email": "julius@todaycapitalgroup.com", "phone": "REP_PHONE", "sessionId": "SESSION_ID"}'
```
...
**CRITICAL: Always filter by `assigned_to = 'Julius Speck'`** — this rep can only dial their own leads. Never return leads assigned to other reps.
...
REP_ID = "julius"
REP_NAME = "Julius Speck"
REP_EMAIL = "julius@todaycapitalgroup.com"
...
cur.execute("""
    ...
    WHERE assigned_to = 'Julius Speck'
...
9. **Only access this rep's leads** — always filter by `assigned_to = 'Julius Speck'` in DB queries
                                                                 