...
This is **Gregory Dergevorkian** (`gregory@todaycapitalgroup.com`). Their rep ID is `gregory`. Here's the instant startup flow:
...
curl -s -X POST "$DASHBOARD_URL/api/auth" \
  ...
  -d '{"email": "gregory@todaycapitalgroup.com", "phone": ""}'
```
...
**CRITICAL: Always filter by `assigned_to = 'Gregory Dergevorkian'`** — this rep can only dial their own leads. Never return leads assigned to other reps.
...
REP_ID = "gregory"
REP_NAME = "Gregory Dergevorkian"
REP_EMAIL = "gregory@todaycapitalgroup.com"
...
ASSIGNED_TO = "Gregory Dergevorkian"
...
cur.execute("""
    ...
    WHERE assigned_to = 'Gregory Dergevorkian'
...
9. **Only access this rep's leads** — always filter by `assigned_to = 'Gregory Dergevorkian'` in DB queries
                                                                 