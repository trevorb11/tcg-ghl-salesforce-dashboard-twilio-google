...
export default function DialerDashboard({ rep, leads, onEnd, sessionId: initialSessionId }: {
  ...
  return (
    <div className="max-w-7xl mx-auto p-4 sm:p-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-medium">
              {status === "on_call" ? (callLog.length > 0 && callLog[callLog.length - 1]?.disposition === "voicemail" ? "Voicemail" : "Live Call") :
        ...
        <div className="lg:col-span-2 space-y-4">
          ...
          {sessionId && !["idle","ended","paused"].includes(status) && (
            <div className="bg-gray-900 border border-gray-800 rounded-xl p-6">
              ...
              {status === "on_call" && (() => {
                const lastCall = callLog.length > 0 ? callLog[callLog.length - 1] : null;
                const isVoicemail = lastCall?.disposition === "voicemail";
                return (
                <div className="flex items-center justify-center gap-3 py-3">
                  {isVoicemail ? (
                    <>
                      <div className="w-3 h-3 bg-purple-500 rounded-full animate-pulse" />
                      <span className="text-purple-400 font-medium">Voicemail{currentLead ? ` — ${currentLead.name}` : ""}</span>
                    </>
                  ) : (
                    <>
      ...
      {/* Floating DTMF dialpad — always visible bottom right */}
      {sessionId && status !== "idle" && status !== "ended" && (
        <div className="fixed bottom-4 right-4 z-40">
          <div className="bg-gray-900/95 border border-gray-700 rounded-xl shadow-2xl p-3 w-44 backdrop-blur-sm">
            <p className="text-[10px] text-gray-500 uppercase tracking-wider mb-2 text-center">Dialpad</p>
            <div className="grid grid-cols-3 gap-1.5">
              {["1","2","3","4","5","6","7","8","9","*","0","#"].map(digit => (
                <button
                  ...
                  className="py-2 bg-gray-800 hover:bg-gray-700 text-white text-sm font-medium rounded-lg transition-all duration-100 active:scale-90 active:bg-gray-600"
                >
                  {digit}
                </button>
              ))}
